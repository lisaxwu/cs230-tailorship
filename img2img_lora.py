...
def parse_args():
    ...
    parser.add_argument("--base_lora_path", type=str, default=None, help="Base LoRA path.")
    parser.add_argument(
        "--prediction_type",
        type=str,
        default=None,
...
def main(args):
    ...
    # Maybe load existing lora weights
    if args.base_lora_path:
        print("Load existing loras!!!", args.base_lora_path)
        lora_weights = load_file(args.base_lora_path)
    ...
    train_dataset = JoinedDataset(transform=train_transforms, tokenizer=tokenizer)
    ...
    def collate_fn(examples):
        ...
        base_pixel_values = torch.stack([example["base"] for example in examples])
        base_pixel_values = base_pixel_values.to(memory_format=torch.contiguous_format).float()
        input_ids = torch.stack([example["prompt_ids"] for example in examples])
        return {"input_pixel_values": input_pixel_values, "target_pixel_values": target_pixel_values, "base_pixel_values": base_pixel_values, "input_ids": input_ids}
    ...
    print('########## Img2Img UNet Lora! Uses input+base - target latents as noise!')
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                base_latents = vae.encode(batch["base_pixel_values"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                noise = torch.sub(input_latents, target_latents)
                if args.noise_offset:
                    # https://www.crosslabs.org//blog/diffusion-with-offset-noise
                    noise += args.noise_offset * torch.randn(
                        (input_latents.shape[0], input_latents.shape[1], 1, 1), device=input_latents.device
                    )
